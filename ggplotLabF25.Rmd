...
```{r}
heights %>% ggplot(aes(x=height)) + geom_histogram() + facet_wrap(~sex)
```
...
heights %>% ggplot(aes(x=height)) + geom_histogram(binwidth = 5)
heights %>% ggplot(aes(x=height)) + geom_histogram(binwidth = 10)
heights %>% ggplot(aes(x=height)) + geom_histogram(binwidth = 20)
...
```{r}
heights %>% ggplot(aes(x=height,group=sex,color=sex,fill=sex,))   + geom_density()
...
```{r}
heights %>% ggplot(aes(x=height,group=sex,color=sex,fill=sex)) + geom_density(alpha=.6)
...
```{r}
heights %>% ggplot(aes(x=height))+geom_boxplot()
```
...
```{r}
heights %>% summarise(avg_height = mean(height),median_height  = median(height))
```
...
heights %>% summarise(avg_height = mean(height),median_height = median(height),.by=sex)
```

How tall is the tallest woman? How short is the shortest man? 

```{R}
heights %>% filter(sex == 'Female') %>% slice_max(height)
heights %>% filter(sex == 'Male') %>% slice_min(height)
print('The tallest woman is 79 inches and the shortest man is 50 inches')
```
...
```{r}
votedata %>% ggplot(aes(x=state,y=demVote)) +geom_col()
...
votedata %>% ggplot(aes(x=state,y=demVote)) +geom_col() + coord_flip()
...
```{r}
votedata %>% ggplot(aes(x=reorder(state,demVote),y=demVote)) +geom_col() + coord_flip()
...
```{r}
votedata %>% ggplot(aes(x=reorder(state,demVote),y=demVote)) +geom_col() + coord_flip() + facet_grid(~south,)
...
```{r}
votedata %>% ggplot(aes(x=reorder(state,demVote),y=demVote,fill=south)) +geom_col() + coord_flip()
...
```{r, eval = F}
votedata %>% ggplot(aes(x=reorder(state,demVote),y=demVote,fill=south)) +geom_col() + coord_flip() +
  labs(title = 'Democratic Voting Patterns in the United States',
       subtitle ='Northern States vs Southern States',
       x = 'Number of Democratic Votes', y = 'States')
...
votedata %>% ggplot(aes(x=reorder(state,demVote),y=demVote,fill=south)) +geom_col() + coord_flip() +
  labs(title = 'Democratic Voting Patterns in the United States',
       subtitle ='Northern States vs Southern States',
       x = 'Number of Democratic Votes', y = 'States') + theme(legend.position ='bottom')
...
```{r}
votedata %>% filter(between(year,2008,2016)) %>%
...
votedata %>% filter(between(year,2008,2016)) %>%
  ...
  labs(title = 'Democratic Voting Patterns in the United States',
       subtitle ='Northern States vs Southern States',
       x = 'Number of Democratic Votes', y = 'States') +
  theme(legend.position ='bottom')+ geom_hline(yintercept = 50)
```
...
```{r}
votedata %>% group_by(year,south) %>% summarise(mean(demVote))
...
votedata %>% group_by(year,south) %>% summarise(avg_vote = mean(demVote)) %>%
ggplot(aes(y=avg_vote,x=year,color=south)) + geom_line() +
  labs(title = 'Average Democratic Vote Over Time',
       subtitle = 'Northern States vs Southern States')
```
...
```{r}
penguins %>%drop_na(body_mass_g) %>%  ggplot(aes(x=body_mass_g,y=species))+geom_boxplot()
...
```{r}
penguins %>% drop_na(body_mass_g) %>% ggplot(aes(x=body_mass_g,y=species))+ geom_boxplot() + geom_point()
...
penguins %>% drop_na(body_mass_g) %>% ggplot(aes(x=body_mass_g,y=species,alpha=.6))+ geom_point()+geom_boxplot()
...
```{r}
penguins %>% drop_na(body_mass_g) %>% ggplot(aes(x=body_mass_g,y=species))+ geom_violin()
...
penguins %>% drop_na(body_mass_g) %>% group_by(island) %>% summarise(mean(body_mass_g))
penguins %>% drop_na(bill_length_mm) %>% group_by(sex) %>% summarise(avg_bill_length_mm = mean(bill_length_mm))

penguins %>% filter(sex=='female') %>% ggplot(aes(x=bill_length_mm,y=bill_depth_mm)) + geom_point()
penguins %>% ggplot(aes(x=flipper_length_mm,y=body_mass_g,color=species))+geom_point()+ facet_wrap(~island)

penguins %>% ggplot(aes(x=flipper_length_mm,color=sex)) + geom_density()
penguins %>% drop_na(body_mass_g) %>% ggplot(aes(x=body_mass_g,color=factor(year)))+ geom_density() + labs(color='Year')
```
