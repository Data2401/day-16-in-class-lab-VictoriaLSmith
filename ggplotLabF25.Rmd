...
```{r}
heights %>% ggplot(aes(x=height)) + geom_histogram(binwidth = 1)+ geom_density()
...
```{r}
heights %>% ggplot(aes(x=height,group=sex,color=sex,fill=sex,alpha=.6)) + geom_histogram(binwidth = 1)  + geom_density()
```
...
```{R}
heights %>% group_by(sex) %>% slice_max(height)
...
```{r}
penguins %>% ggplot(aes(x=body_mass_g))+geom_boxplot(aes(group=species,fill=species))
